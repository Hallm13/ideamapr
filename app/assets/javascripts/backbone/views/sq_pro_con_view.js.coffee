IdeaMapr.Views.SQProConView = IdeaMapr.Views.SurveyQuestionView.extend
  render: (model, options) ->
    # A model for the question view is created when the question is first initialized
    
    template = _.template($('#sq-procon-template').html())
    this.$el.html(template(this.model.attributes))

    if !this.is_shown
      this.$el.addClass 'myhidden'

    idea_list_view = new IdeaMapr.Views.IdeaListProConView
